  ...
  return (
    <div className="steve-page space-y-4">
      <PageHero
        ...
        subtitle={question}
        actions={
          <button type="button" className="rounded-full border border-[var(--color-steve-brief-border)] px-3.5 py-2 text-[12px] text-[var(--color-steve-green-bright)]" onClick={() => setComposerOpen(true)}>
            + {t('actions.newChat')}
          </button>
        }
      />
        ...
        <section className="border-e border-[var(--color-steve-border)] bg-[var(--color-steve-surface)]">
          ...
          <div className="max-h-[62vh] overflow-y-auto">
            {filtered.map((th) => (
              ...
              >
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-[var(--color-steve-border)] text-[11px] text-[var(--color-steve-gold)]">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    {th.unread > 0 ? <span className="rounded-full bg-[var(--color-steve-green)] px-1.5 text-[10px] text-white">{d(th.unread)}</span> : null}
                  </div>
            ...
            <>
              <div className="border-b border-[var(--color-steve-border)] px-5 py-4">
                ...
                <div className="mt-1 text-[12px] text-[var(--color-steve-text-faint)]">
                  ...
                  {active.relatedWork ? (
                    <>
                      {t('communication.relatedWork', { id: '' })}
                      <Ltr>{active.relatedWork}</Ltr>
                    </>
                  ) : null}
                </div>
                <div className="mt-3 flex flex-wrap gap-2">
                  {active.relatedRecordType && active.relatedRecordId ? (
                    <Link to={recordPath(active.relatedRecordType, active.relatedRecordId)} className="rounded-full border border-[var(--color-steve-brief-border)] px-3 py-1 text-[11px] text-[var(--color-steve-green-bright)]">
                      {t('communication.openRecord')}
                    </Link>
                  ) : null}
                  {active.relatedWork ? (
                    <Link to={`/work/${active.relatedWork}`} className="rounded-full border border-[var(--color-steve-border)] px-3 py-1 text-[11px]">
                ...
                {active.messages.map((m) => (
                  <div key={m.id} className={m.from.includes('امینی') ? 'flex justify-start' : 'flex justify-end'}>
                    <div className={cn('max-w-[78%] px-3.5 py-2.5', m.from.includes('امینی') ? 'rounded-2xl rounded-se-md bg-[var(--color-steve-green-dim)]' : 'rounded-2xl rounded-ss-md bg-[var(--color-steve-elevated)]')}>
                      ...
                      <p className="mt-1 text-[13px] leading-7 text-[var(--color-steve-text-muted)]">{m.body}</p>
                      {m.imageSrc ? (
                        <button
                          type="button"
                      ...
                      {m.linkType && m.linkId ? (
                        <button type="button" className="mt-2 inline-flex items-center gap-1 text-[11px] text-[var(--color-steve-gold)]" onClick={() => navigate(recordPath(m.linkType!, m.linkId!))}>
                          {m.linkLabel || t('communication.viewLink')} <Chevron size={12} />
                        </button>
                      ) : null}
                    </div>
                  </div>
              ...
              <div className="border-t border-[var(--color-steve-border)] px-4 py-3">
                <div className="flex items-center gap-2 rounded-full border border-[var(--color-steve-border)] bg-[var(--color-steve-surface)] px-2 py-1.5">
                  <button
                    type="button"
                    className="rounded-full p-2 text-[var(--color-steve-text-faint)]"
                  ...
                  <button
                    type="button"
                    className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-[var(--color-steve-green)] text-white disabled:opacity-40"
                    disabled={!draft.trim()}
                    onClick={() => {
                      dispatch({ type: 'SEND_MESSAGE', threadId: active.id, body: draft.trim() })
                      setDraft('')
